...
const universities = graduates.map((cb) => {
...
const contactInfo = graduates.map((cb) => {
  return `${cb.first_name} ${cb.email}`
})
console.log(contactInfo);
...
const unisWithUni = graduates.indexOf('Uni');
console.log(unisWithUni);
...
const displayNames = zooAnimals.map((cb) => {
...
const lowPopulationAnimals = zooAnimals.filter((cb) => {
  return cb.population < 5;
})
console.log(lowPopulationAnimals);
...
const populationTotal = zooAnimals.reduce((total, cb) => {
  return total += cb.population;
}, 0);
console.log(populationTotal);
